...
do 
  sed -e 's/^    //' << ENDOFYAML | kubectl apply -f -
    ...
    ---
ENDOFYAML
  kubectl -n $ns create --dry-run=client configmap flow-xml --from-file=tests/06-$ns.flow.json -o yaml | kubectl apply -f -
done
...
helm -n alpha install nifi . \
  ...
  --set customFlow=/opt/nifi/flow-xml/06-alpha.flow.json \
...
helm -n bravo install nifi . \
  ...
  --set configmaps[0].name=flow-xml \
  --set configmaps[0].mountPath=/opt/nifi/flow-xml \
  --set customFlow=/opt/nifi/flow-xml/06-bravo.flow.json \
  --set certManager.caDuration=1h \
  --set certManager.refreshSeconds=30 \
